...
CREATE TABLE IF NOT EXISTS entries (
  ...
  , debit_precision    INT NOT NULL
  , credit_amount      INT NOT NULL
  , credit_precision   INT NOT NULL
);
...
COMMENT ON COLUMN entries.debit_amount IS 'The value increase/decrease (base integer) on the debit side.';
COMMENT ON COLUMN entries.debit_precision IS 'The number of decimal places for the debit_amount (e.g., 2 for cents).';
COMMENT ON COLUMN entries.credit_amount IS 'The value increase/decrease (base integer) on the credit side.';
COMMENT ON COLUMN entries.credit_precision IS 'The number of decimal places for the credit_amount (e.g., 2 for cents).';
